-- List a site's members without going through site_members RLS.
-- User-JWT SELECTs on site_members often return only the caller's row
-- (or hang), so team pickers show "You" and hide everyone else.
...
COMMENT ON FUNCTION public.get_my_site_members(uuid) IS
  'All site_members for a site the current user owns or belongs to. Bypasses RLS to avoid own-row-only SELECT.';
