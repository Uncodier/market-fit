...
export async function GET(request: NextRequest) {
  ...
  const reservedOutputTokens = data.reserved_output_tokens ?? (availableTokens ? Math.max(2048, Math.ceil(availableTokens * .1)) : 0)
  return NextResponse.json({ context: {
    model: data.model, provider: data.provider, usedTokens: data.used_tokens || 0,
    outputTokens: data.output_tokens || 0, availableTokens, reservedOutputTokens,
