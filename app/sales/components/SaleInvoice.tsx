...
const SOURCE_STYLES: Record<string, string> = {
  ...
  shop: "bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border-indigo-200",
  marketplace: "bg-violet-50 text-violet-700 hover:bg-violet-100 border-violet-200",
...
export function SaleInvoice({
  ...
  const formatDate = (dateString: string) => {
    try {
      const activeLocale = dateFnsLocales[locale as keyof typeof dateFnsLocales] || enUS
      return format(new Date(dateString), 'PP', { locale: activeLocale })
    } catch {
      return dateString
    }
  }
  ...
  const computedStatus = sale.amount_due === 0 && sale.status === 'pending' ? 'completed' : sale.status;

  // Derive the display payment method from the latest payment if available
  const displayPaymentMethod = () => {
    if (sale.payments && sale.payments.length > 0) {
      const latestPayment = [...sale.payments].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
  ...
  return (
    <div className="bg-card dark:bg-card rounded-lg shadow-lg overflow-hidden border border-border dark:border-border" style={{
        ...
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-semibold text-foreground dark:text-foreground">
              {sale.title && sale.title.startsWith("Order - ") 
                ? sale.title.replace("Order", t('sales.detail.orderNumber') || "Order") 
                : sale.title}
            </h2>
            {sale.description && (
      ...
      {/* Key facts strip — full width, like other invoice documents */}
      <div className="bg-muted/50 dark:bg-muted/10 p-4 border-b border-border dark:border-border">
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            ...
            <div className="text-lg font-bold text-primary dark:text-primary">{formatCurrency(sale.amount)}</div>
            {sale.currency && (
              <div className="text-xs text-muted-foreground dark:text-muted-foreground">{sale.currency}</div>
            )}
          </div>
          <div>
            <div className="text-xs text-muted-foreground dark:text-muted-foreground mb-1">
            ...
            <Badge className={`${STATUS_STYLES[computedStatus] || STATUS_STYLES.pending} mt-0.5`}>
          ...
          <div>
            ...
            <Badge className={`${SOURCE_STYLES[sale.source] || SOURCE_STYLES.online} mt-0.5`}>
      ...
      <div className="p-6 border-b border-border dark:border-border">
        <div className={`grid gap-6 ${!saleOrder ? "md:grid-cols-2" : "md:grid-cols-1"}`}>
          {!saleOrder && (
            <div>
                ...
                <table className="w-full text-sm">
                  <tbody>
                    <tr className="border-b border-border dark:border-border">
                      <td className="py-2 px-3 bg-muted/50 dark:bg-muted/10 font-medium w-1/3">
              ...
              <table className="w-full text-sm">
                <tbody>
                  <tr className="border-b border-border dark:border-border">
                    ...
                    <td className="py-2 px-3">{formatPaymentMethod(displayPaymentMethod())}</td>
                  </tr>
                  <tr className="border-b border-border dark:border-border">
                    <td className="py-2 px-3 bg-muted/50 dark:bg-muted/10 font-medium">
                      {t('sales.table.date') || "Date"}
                    ...
                    <tbody>
                      {sale.payments.map((payment, index) => (
                        <tr
                          ...
                          className={index < sale.payments!.length - 1 ? "border-b border-border dark:border-border" : ""}
                        >
        ...
        <div className="p-6 border-b border-border dark:border-border">
          ...
          <div className="mb-4">
            <div className="text-base font-medium flex items-center gap-2 flex-wrap">
              ...
              <button
                onClick={() => navigateToOrder({ orderId: saleOrder.id, orderNumber: saleOrder.orderNumber, router })}
                className="text-xs font-normal text-blue-600 hover:underline flex items-center gap-1 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100 cursor-pointer"
              >
