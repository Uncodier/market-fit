  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (profileData) {
        ...
        const et = profileData.settings?.calendar?.event_types?.find(
          (e) => e.slug === eventSlug,
        );
        if (et) {
          setEventType(et);
                      ...
                      <div className="flex-1 flex flex-col animate-in fade-in duration-300 max-w-md mx-auto w-full">
                        ...
                        <div className="grid grid-cols-7 gap-y-4 gap-x-1">
                          ...
                          {days.map((date) => {
                            ...
                            // Only check if it's explicitly loaded as true/false or if we are still loading, assume true for style reasons or we can be explicit
