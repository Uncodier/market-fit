...
import { getProfileBySlug, getAvailableSlots, bookMeeting, getSiteInfoBySlug } from "@/app/book/actions"
...
import { CalendarIcon, Clock, ChevronLeft, ChevronRight, CheckCircle2, Globe, Sun, Moon } from "@/app/components/ui/icons"
...
export default function UserBookingPage() {
  ...
  useEffect(() => {
    async function loadData() {
      const [profileData, sInfo] = await Promise.all([
        getProfileBySlug(userSlug),
        getSiteInfoBySlug(siteSlug)
      ])
      
      if (profileData) {
        setProfile(profileData)
        const et = profileData.settings?.calendar?.event_types?.find(e => e.slug === eventSlug)
        if (et) {
          setEventType(et)
        }
      }
    ...
    loadData()
  }, [userSlug, siteSlug, eventSlug])

  useEffect(() => {
    if (selectedDate && profile?.id && eventType) {
        ...
        <Card className="max-w-md w-full">
          <CardContent className="pt-10 pb-10 text-center space-y-4">
            ...
            <div className="pt-4 border-t border-border mt-6 text-left space-y-2">
              ...
              <p className="text-sm"><strong>{t('booking.details.with')}:</strong> {profile.name || userSlug}</p>
              <p className="text-sm"><strong>{t('booking.details.event')}:</strong> {eventType.title}</p>
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="space-y-4">
              {profile.avatar_url && (
                <img 
                  ...
                  alt={profile.name} 
                  className="h-20 w-20 rounded-full object-cover border-2 border-primary/10 shadow-sm"
                />
              )}
              <div>
