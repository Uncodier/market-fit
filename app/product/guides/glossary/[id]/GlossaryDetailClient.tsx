      ...
      <section className="relative w-full pt-24 pb-16 border-b dark:border-white/[0.04] border-black/5 dark:bg-black-paper bg-white-paper bg-white">
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-6 lg:px-12">
          <Link href="/product/guides?category=Glossary" className="inline-flex items-center text-sm font-medium text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white mb-8 transition-colors">
            ...
            {t('guides.glossary.back') || 'Back to Glossary'}
          </Link>
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
            ...
            <div className="flex-1">
              ...
              <div className={`font-medium mb-4 flex items-center gap-2 ${theme.textAccent}`}>
                <span className="font-semibold uppercase tracking-wider text-xs px-2 py-0.5 rounded border border-current">{t(`guides.category.${termData.category?.toLowerCase() || ''}`) || termData.category}</span>
              </div>
      ...
      <section className="py-20 max-w-5xl mx-auto px-6 lg:px-12 w-full flex-1 flex flex-col md:flex-row gap-12">
        <div className="flex-1">
          <div className="prose prose-slate dark:prose-invert max-w-none text-slate-600 dark:text-slate-300">
            <h2 className="text-2xl font-bold mb-6 text-slate-900 dark:text-white">{t('guides.glossary.understanding') || 'Understanding '}{termData.term.split(' ')[0]}</h2>
            <p>
              {t('guides.glossary.desc1') || 'This concept is a core element in modern Go-To-Market strategies. When effectively implemented, it allows teams to align their efforts and measure success predictably.'}
            </p>
            
            <h3 className="text-xl font-bold mt-8 mb-4 text-slate-900 dark:text-white">{t('guides.glossary.whyItMatters') || 'Why it matters'}</h3>
            <p>
              {t('guides.glossary.desc2_1') || 'Understanding and mastering '}<strong>{termData.term}</strong>{t('guides.glossary.desc2_2') || ' enables your team to optimize conversion rates, reduce friction in the buyer journey, and ultimately scale your business more efficiently.'}
            </p>
            
            <h3 className="text-xl font-bold mt-8 mb-4 text-slate-900 dark:text-white">{t('guides.glossary.related') || 'Related Strategies'}</h3>
            <p className="text-slate-600 dark:text-slate-300">
              {t('guides.glossary.desc3') || 'For practical ways to apply this within your own workflow, check out our related guides or reach out to see how Makinari can automate these processes for you.'}
            </p>
          </div>
        </div>
        ...
        <div className="w-full md:w-80 flex-shrink-0">
          <div className="p-6 rounded-2xl dark:bg-[#0f0f13] bg-slate-50 border dark:border-white/5 border-black/5">
            <h3 className="font-bold text-lg mb-4 text-slate-900 dark:text-white">{t('guides.glossary.details') || 'Term Details'}</h3>
            
            <div className="space-y-4 text-sm">
              <div className="flex justify-between items-center py-2 border-b dark:border-white/5 border-black/5">
                <span className="text-slate-500">{t('guides.glossary.domain') || 'Domain'}</span>
                <span className="font-medium text-slate-900 dark:text-white">{t(`guides.category.${termData.category?.toLowerCase() || ''}`) || termData.category}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b dark:border-white/5 border-black/5">
                <span className="text-slate-500">{t('guides.glossary.complexity') || 'Complexity'}</span>
                <span className="font-medium text-slate-900 dark:text-white">{t('guides.glossary.fundamental') || 'Fundamental'}</span>
              </div>
            </div>
            
            <div className="mt-8 pt-6 border-t dark:border-white/5 border-black/5">
              <h4 className="font-bold text-slate-900 dark:text-white mb-3">{t('guides.glossary.moreHelp') || 'Want more help?'}</h4>
              <Link href="#" className={`flex items-center text-sm font-medium transition-colors ${theme.textAccent} ${theme.hoverText}`}>
                {t('guides.glossary.talkToExpert') || 'Talk to a GTM expert'}
