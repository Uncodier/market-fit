      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        <div>
          <h2 className="text-3xl md:text-4xl font-bold dark:text-white text-slate-900 mb-4 tracking-tight">
            {t('guides.grid.title') || 'Explore Guides'}
          </h2>
          <p className="dark:text-white/50 text-slate-500 text-lg max-w-xl">
            {t('guides.grid.subtitle') || 'Actionable tactics and comprehensive resources for growth.'}
          </p>
        </div>
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            placeholder={isGlossary ? (t('guides.grid.searchTerms') || "Search terms...") : (t('guides.grid.searchGuides') || "Search guides...")}
            value={searchQuery}
        ...
        <div className="w-full lg:w-48 flex-shrink-0">
          <div className="flex lg:flex-col overflow-x-auto lg:overflow-visible pb-4 lg:pb-0 gap-1.5 scrollbar-hide">
            {categories.map(category => (
              ...
              >
                {t(`guides.category.${category.toLowerCase()}`) || category}
              </button>
            ))}
          </div>
        </div>
          ...
          {isGlossary ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredTerms.length > 0 ? (
                ...
                <div className="col-span-full py-20 flex flex-col items-center justify-center text-center border border-dashed dark:border-white/10 border-black/10 rounded-2xl">
                  ...
                  <h3 className="text-lg font-bold dark:text-white text-slate-900 mb-2">{t('guides.grid.noTermsFound') || 'No terms found'}</h3>
                  <p className="dark:text-slate-400 text-slate-500 max-w-sm">
                    {t('guides.grid.noTermsMatch1') || 'We couldn\'t find any terms matching "'}{searchQuery}{t('guides.grid.noTermsMatch2') || '". Try adjusting your search term.'}
                  </p>
                </div>
              )}
            </div>
            ...
            filteredGuides.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-3 flex flex-wrap items-center gap-1.5">
                        {guide.categories?.filter(c => c !== 'All').map((cat, i) => (
                          <span key={i} className="uppercase tracking-wider px-2 py-0.5 rounded bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-[10px]">
                            {t(`guides.category.${cat.toLowerCase()}`) || cat}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="mt-6 flex items-center text-sm font-bold opacity-0 group-hover:opacity-100 transition-all -translate-x-2 group-hover:translate-x-0 duration-300">
                        {t('guides.grid.readGuide') || 'Read guide'}
              ...
              <div className="w-full py-20 flex flex-col items-center justify-center text-center border border-dashed dark:border-white/10 border-black/10 rounded-2xl">
                ...
                <h3 className="text-lg font-bold dark:text-white text-slate-900 mb-2">{t('guides.grid.noGuidesFound') || 'No guides found'}</h3>
                <p className="dark:text-slate-400 text-slate-500 max-w-sm">
                  {t('guides.grid.noGuidesMatch1') || 'We couldn\'t find any guides matching "'}{searchQuery}{t('guides.grid.noGuidesMatch2') || '". Try adjusting your filters or search term.'}
                </p>
              </div>
            )
          )}
