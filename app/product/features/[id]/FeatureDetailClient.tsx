  ...
  return (
    <div className="relative w-full dark:bg-[#030303] bg-white dark:text-white text-slate-900 selection:bg-violet-500/30 flex flex-col font-sans overflow-hidden min-h-screen">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
            ...
            <div className="flex-1">
              ...
              <div className={`font-medium mb-4 flex items-center gap-2 ${theme.textAccent}`}>
                <span className="font-semibold uppercase tracking-wider text-xs px-2 py-0.5 rounded border border-current">{feature.stage}</span>
                <span>•</span>
      ...
      <section className="py-20 max-w-5xl mx-auto px-6 lg:px-12 w-full flex-1 flex flex-col md:flex-row gap-12">
        <div className="flex-1">
          <h2 className="text-2xl font-bold mb-6 text-slate-900 dark:text-white">About this feature</h2>
          <div className="prose prose-slate dark:prose-invert max-w-none text-slate-600 dark:text-slate-300">
            <p>
              {feature.name} is a core capability within the {feature.stage} stage of Makinari. 
              Designed to help revenue teams work more efficiently, this feature integrates seamlessly 
              with the rest of the platform to create a unified workflow from end to end.
            </p>
            
            <h3 className="text-xl font-bold mt-8 mb-4 text-slate-900 dark:text-white">Key Capabilities</h3>
            <ul className="space-y-3 mb-8">
              {[1, 2, 3].map((item) => (
                <li key={item} className="flex items-start">
                  <CheckCircle2 size={20} className={`${theme.checkColor} mr-3 mt-0.5 flex-shrink-0`} />
                  <span className="text-slate-600 dark:text-slate-300">
                    Streamline your workflow and increase productivity with {feature.name} automation capabilities.
                  </span>
                </li>
              ))}
            </ul>
            ...
            <p className="text-slate-600 dark:text-slate-300">
              When using {feature.name}, teams typically see a significant reduction in manual work 
        ...
        <div className="w-full md:w-80 flex-shrink-0">
          <div className="p-6 rounded-2xl dark:bg-[#0f0f13] bg-slate-50 border dark:border-white/5 border-black/5">
            <h3 className="font-bold text-lg mb-4 text-slate-900 dark:text-white">Feature Details</h3>
            
            <div className="space-y-4 text-sm">
              <div className="flex justify-between items-center py-2 border-b dark:border-white/5 border-black/5">
                <span className="text-slate-500">Stage</span>
                <span className="font-medium text-slate-900 dark:text-white">{feature.stage}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b dark:border-white/5 border-black/5">
              ...
              <div className="flex justify-between items-center py-2">
                ...
                <span className="font-medium capitalize text-slate-900 dark:text-white">{feature.status.replace('_', ' ')}</span>
              </div>
            </div>
