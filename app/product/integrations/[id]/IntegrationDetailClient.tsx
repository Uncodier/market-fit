  ...
  return (
    <div className="relative w-full dark:bg-[#030303] bg-white dark:text-white text-slate-900 selection:bg-violet-500/30 flex flex-col font-sans overflow-hidden min-h-screen">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
            ...
            <div className="w-full md:w-auto mt-4 md:mt-0 flex flex-col sm:flex-row md:flex-col gap-3">
              {integration.status === 'coming_soon' ? (
                ...
                <Link 
                  href="/auth"
                  className="px-8 py-3.5 rounded-xl font-bold transition-all shadow-sm flex items-center justify-center bg-emerald-500 hover:bg-emerald-600 text-white shadow-[0_0_20px_rgba(16,185,129,0.3)]"
                >
                  Connect Integration
                </Link>
              )}
      ...
      <section className="py-20 max-w-5xl mx-auto px-6 lg:px-12 w-full flex-1 flex flex-col md:flex-row gap-12">
        <div className="flex-1">
          <h2 className="text-2xl font-bold mb-6 text-slate-900 dark:text-white">About this integration</h2>
          <div className="prose prose-slate dark:prose-invert max-w-none text-slate-600 dark:text-slate-300">
            <p>
              Connect Makinari with {integration.name} to streamline your workflow and automate tasks across both platforms. 
              Our deep integration allows you to sync data in real-time, trigger actions based on events, and manage 
              everything from a single dashboard.
            </p>
            
            <h3 className="text-xl font-bold mt-8 mb-4 text-slate-900 dark:text-white">Key Features</h3>
            <ul className="space-y-3 mb-8">
              {[1, 2, 3].map((item) => (
                <li key={item} className="flex items-start">
                  <CheckCircle2 size={20} className="text-emerald-500 mr-3 mt-0.5 flex-shrink-0" />
                  <span className="text-slate-600 dark:text-slate-300">
                    Sync your {integration.name} data with Makinari in real-time to keep your team aligned.
                  </span>
                </li>
              ))}
            </ul>
            
            <h3 className="text-xl font-bold mt-8 mb-4 text-slate-900 dark:text-white">How it works</h3>
            <p className="text-slate-600 dark:text-slate-300">
              Setting up the {integration.name} integration takes just a few clicks. Once connected, 
              you can configure specific rules and triggers to customize how data flows between the two platforms.
            </p>
          </div>
        </div>
        ...
        <div className="w-full md:w-80 flex-shrink-0">
          <div className="p-6 rounded-2xl dark:bg-[#0f0f13] bg-slate-50 border dark:border-white/5 border-black/5">
            <h3 className="font-bold text-lg mb-4 text-slate-900 dark:text-white">Integration Details</h3>
            
            <div className="space-y-4 text-sm">
              <div className="flex justify-between items-center py-2 border-b dark:border-white/5 border-black/5">
                <span className="text-slate-500">Developer</span>
                <span className="font-medium text-slate-900 dark:text-white">Makinari Team</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b dark:border-white/5 border-black/5">
                <span className="text-slate-500">Category</span>
                <span className="font-medium text-slate-900 dark:text-white">{integration.category}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b dark:border-white/5 border-black/5">
                <span className="text-slate-500">Pricing</span>
                <span className="font-medium text-slate-900 dark:text-white">Included in all plans</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-slate-500">Requirements</span>
                <span className="font-medium text-slate-900 dark:text-white">{integration.name} Account</span>
              </div>
            </div>
