    ...
    try {
      if (authMode === 'sign_up') {
        ...
        if (data.user?.email_confirmed_at) {
          ...
          setErrorMessage("Check your email for a confirmation link to complete your account setup.")
        }
      } else {
  ...
  const onResetPasswordSubmit = async (values: ResetPasswordValues) => {
    ...
    try {
      const { error } = await supabase.auth.resetPasswordForEmail(values.email, {
        redirectTo: `${window.location.origin}/auth/set-password?returnTo=${encodeURIComponent(finalReturnTo)}`
      })
