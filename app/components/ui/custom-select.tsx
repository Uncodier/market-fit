  ...
  ({ className, containerClassName, icon, children, ...props }, ref) => {
    return (
      <div className={cn("relative", containerClassName)}>
        ...
        <select
          className={cn(
            "h-10 w-full rounded-md border dark:border-white/5 border-black/5 bg-background px-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-inset focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50",
            icon && "pl-10 pr-8",
            !icon && "pr-8",
            className
          )}
