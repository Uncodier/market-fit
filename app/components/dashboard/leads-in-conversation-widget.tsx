...
interface LeadsInConversationWidgetProps {
  startDate: Date;
  endDate: Date;
  segmentId?: string;
}

interface LeadsInConversationData {
  actual: number;
  percentChange: number;
  periodType: string;
}

export function LeadsInConversationWidget({ 
  startDate, 
  endDate, 
  segmentId = "all" 
}: LeadsInConversationWidgetProps) {
  const [data, setData] = useState<LeadsInConversationData | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetchWithController(
          `/api/performance/leads-in-conversation?${params}`
        );
        ...
        console.error("Error fetching leads in conversation data:", err);
