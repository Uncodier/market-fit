...
export function MarketingCostsWidget({ 
    ...
    const fetchMarketingCosts = async () => {
      ...
      try {
        ...
        if (segmentId && segmentId !== "all") {
          ...
          console.log(`[Marketing Costs Widget] Applying segment filter: ${segmentId}`);
        }

        const response = await fetch(`/api/costs?${queryParams.toString()}`);
        
        if (!response.ok) {
          throw new Error('Failed to fetch marketing costs data');
        }
        
        const data = await response.json();
        
        // Find marketing category from cost categories
        const marketingCategory = data.costCategories?.find(
          (cat: any) => cat.name === "Marketing"
        );
        
        if (marketingCategory) {
          setMarketingCosts(marketingCategory.amount || 0);
          setPercentChange(marketingCategory.percentChange || 0);
          setPeriodType(data.periodType || "monthly");
          
          console.log(`[Marketing Costs Widget] Received marketing costs: $${marketingCategory.amount}, segment: ${segmentId || 'all'}`);
        } else {
          throw new Error('Marketing category not found in cost data');
        }
      } catch (error) {
        console.error('Error fetching marketing costs:', error);
        setMarketingCosts(2500); // Default fallback value
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Format the marketing costs value as currency
  ...
  const formattedValue = formatCurrency(marketingCosts);
  ...
  return (
    <BaseKpiWidget
      ...
      tooltipText={`Total marketing expenses${segmentId && segmentId !== "all" ? " for this segment" : ""}`}
