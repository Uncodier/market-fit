...
export function ServiceExclusionsAddressesSection({ onSave }: ServiceExclusionsAddressesSectionProps) {
  ...
  return (
    <div id="service-exclusions-addresses" className="space-y-6">
      {/* Header Section */}
      ...
      {excludedAddresses.map((item, index) => {
        ...
        return (
          <Card key={key} id={`service-exclusion-${index}`} className="border dark:border-white/5 border-black/5">
            {/* Collapsible Header */}
            <CardHeader 
            ...
            {isExpanded && (
              <>
              <CardContent className="space-y-6 px-8 pt-8 pb-8 border-t bg-red-50/30 dark:bg-red-950/10">
                <FormField
              ...
              <ActionFooter>
                <div className="flex items-center gap-2">
                  ...
                  >
                    {isSaving && isSavingAddress === key ? "Saving..." : "Save Exclusion"}
                  </Button>
                </div>
              </ActionFooter>
              </>
