-- SAFE Consolidated Service Role Fix (Simplified Version)
-- Date: 2025-01-30
-- Fixes 406 errors for service_role token queries
-- PREVENTS performance issues from the original script
...
CREATE POLICY "visitors_unified" ON public.visitors FOR ALL USING (
  auth.is_service_role_or_user_condition(
    (visitors.segment_id IS NOT NULL AND EXISTS (
      ...
      WHERE s.id = visitors.segment_id AND sm.user_id = auth.uid() AND sm.status = 'active'
    ))
    OR
    (visitors.lead_id IS NOT NULL AND EXISTS (
      SELECT 1 FROM public.leads l JOIN public.site_members sm ON sm.site_id = l.site_id 
      WHERE l.id = visitors.lead_id AND sm.user_id = auth.uid() AND sm.status = 'active'
    ))
    OR
    EXISTS (
      SELECT 1 FROM public.visitor_sessions vs JOIN public.site_members sm ON sm.site_id = vs.site_id 
      WHERE vs.visitor_id = visitors.id AND sm.user_id = auth.uid() AND sm.status = 'active'
    )
  )
);
...
CREATE POLICY "visitor_sessions_unified" ON public.visitor_sessions FOR ALL USING (
  auth.is_service_role_or_user_condition(
    EXISTS (
      ...
      WHERE sm.site_id = visitor_sessions.site_id AND sm.user_id = auth.uid() AND sm.status = 'active'
    )
  )
);
...
-- ============================================================================
-- STEP 4: ADD ONLY NECESSARY INDEXES (NO DUPLICATES)
-- ============================================================================
...
BEGIN
    ...
    IF problem_count = 0 THEN
        RAISE NOTICE '🎉 SUCCESS: All tables have single policies';
        RAISE NOTICE '✅ Service_role access enabled';
        RAISE NOTICE '✅ No performance warnings expected';
    ELSE
        RAISE NOTICE '❌ PROBLEMS DETECTED: % tables have multiple policies', problem_count;
    END IF;
END $$;
